<x-app-layout>
    ...
    <div class="p-8 space-y-8" style="background-color: #f7f9fb; min-height: 100vh;">
        ...
        <!-- Header -->
        <header>
            <h1 class="font-headline text-4xl font-extrabold tracking-tight mb-2" style="color: #191c1e;">Edit Asset</h1>
            <p class="font-medium" style="color: #464554;">Perbarui detail item dalam koleksi inventaris.</p>
        </header>
        ...
        <div class="max-w-6xl mx-auto">
            <div class="glass-panel rounded-xl shadow-2xl flex flex-col md:flex-row overflow-hidden">
                ...
                <div class="w-full md:w-1/3 p-8 flex flex-col" style="background-color: #f2f4f6;">
                    <h3 class="font-headline font-bold mb-6" style="color: #191c1e;">Preview & Update Media</h3>
                    <div class="flex-1 flex flex-col gap-6">
                        <div class="relative group aspect-square rounded-xl overflow-hidden bg-white shadow-inner border-2 border-dashed border-outline-variant flex flex-col items-center justify-center text-center px-6">
                            @if($alat->gambaralat)
                                <img id="current-img" src="{{ asset('storage/' . $alat->gambaralat) }}" alt="{{ $alat->namaalat }}" class="absolute inset-0 w-full h-full object-cover">
                            @else
                                <div class="flex flex-col items-center">
                                    <span class="material-symbols-outlined text-4xl mb-2 drop-shadow-sm" style="color: #4648d4;">image_not_supported</span>
                                    <p class="text-xs font-bold uppercase tracking-tighter" style="color: #191c1e;">No Image</p>
                                </div>
                            @endif
                            <div class="relative z-10 flex flex-col items-center opacity-0 group-hover:opacity-100 transition-opacity">
                                ...
                                <p class="text-xs font-bold uppercase tracking-tighter" style="color: #191c1e;">Replace Image</p>
                                <p class="text-[10px] mt-1" style="color: #464554;">PNG, JPG hingga 5MB</p>
                            </div>
                        </div>
                        <div class="space-y-4">
                            <div class="p-4 rounded-xl bg-white/50 border" style="border-color: rgba(118, 117, 134, 0.3);">
                                <div class="flex items-center gap-3 mb-2">
                                    <span class="material-symbols-outlined text-sm" style="color: #006387;">edit</span>
                                    <span class="text-[10px] font-bold uppercase tracking-widest" style="color: #464554;">Update Note</span>
                                </div>
                                <p class="text-xs leading-relaxed italic" style="color: #464554;">
                                    "Pastikan spesifikasi akurat untuk log peminjaman dan audit internal."
                                </p>
                            </div>
                        </div>
                    </div>
                ...
                <div class="flex-1 p-8 md:p-12 overflow-y-auto">
                    <div class="flex justify-between items-start mb-8">
                        <div>
                            <h2 class="font-headline text-3xl font-extrabold tracking-tight" style="color: #191c1e;">Update Detail</h2>
                            <p class="font-medium text-sm mt-1" style="color: #464554;">Sempurnakan data item dengan presisi.</p>
                        </div>
                    </div>
                    ...
                    <form method="POST" action="{{ route('alat.update', $alat) }}" enctype="multipart/form-data" class="space-y-6">
                        ...
                        <div class="grid grid-cols-1 gap-6">
                            <div class="flex flex-col gap-2">
                                ...
                                <input id="namaalat" name="namaalat" type="text" class="w-full border-none border-b-2 border-transparent focus:border-primary rounded-xl px-4 py-3 font-medium transition-all focus:ring-0" style="background-color: #e6e8ea;" value="{{ old('namaalat', $alat->namaalat) }}" required autofocus />
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                            ...
                            <div class="flex flex-col gap-2">
                                ...
                                <input id="qty" name="qty" type="number" min="0" class="w-full border-none border-b-2 border-transparent focus:border-primary rounded-xl px-4 py-3 font-medium transition-all focus:ring-0" style="background-color: #e6e8ea;" value="{{ old('qty', $alat->qty) }}" required />
                        ...
                        <div class="flex flex-col gap-2">
                            ...
                            <textarea id="spesifikasi" name="spesifikasi" rows="4" class="w-full border-none border-b-2 border-transparent focus:border-primary rounded-xl px-4 py-3 font-medium transition-all focus:ring-0 resize-none" style="background-color: #e6e8ea;">{{ old('spesifikasi', $alat->spesifikasi) }}</textarea>
                            <x-input-error for="spesifikasi" />
                        </div>

                        <div class="flex flex-col gap-2">
                            <label class="text-xs font-bold uppercase tracking-widest ml-1" style="color: #767586;">Gambar Baru (Opsional)</label>
                            ...
                            <div class="w-full border-2 border-dashed rounded-xl px-4 py-8 text-center cursor-pointer hover:border-primary transition-all" style="border-color: rgba(118, 117, 134, 0.5);" onclick="document.getElementById('gambaralat').click()">
                                ...
                                <p class="text-sm font-medium" style="color: #191c1e;">Klik untuk ganti gambar</p>
                                <p class="text-xs" style="color: #464554;">Gambar lama akan otomatis terhapus</p>
                            </div>
                        ...
                        <div class="pt-4">
                            <label class="text-xs font-bold uppercase tracking-widest ml-1 block mb-4" style="color: #767586;">Status Stok Saat Ini</label>
                            <div class="h-2 w-full rounded-full overflow-hidden flex" style="background-color: #e0e3e5;">
                                <div class="h-full" style="width: {{ ($alat->qty / 100) }}%; background-color: {{ $alat->qty == 0 ? '#ba1a1a' : ($alat->qty < 10 ? '#f59e0b' : '#10b981') }};"></div>
                            </div>
                            <div class="flex justify-between mt-2">
                                <span class="text-[10px] font-bold" style="{{ $alat->qty == 0 ? 'color: #ba1a1a' : ($alat->qty < 10 ? 'color: #f59e0b' : 'color: #10b981') }}">{{ $alat->qty }} Unit</span>
                                <span class="text-[10px] font-bold" style="color: #767586;">{{ $alat->qty == 0 ? 'Habis' : 'Tersedia' }}</span>
                            </div>
                        </div>
